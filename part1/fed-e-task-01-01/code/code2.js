...
//
// 获取对象最后一条数据
let last_car = obj => fp.last(obj);
// 函数组成获取最后一条数据的指定属性值
let isLastInStock = fp.flowRight(getValue("in_stock"), last_car);
// fp.flowRight()
// 2.打印测试
...
// 2.打印测试
let first_car = obj => fp.first(obj);
let firstName = fp.flowRight(getValue("name"), first_car);
...
let averageDollarValue = fp.flowRight(_average, fp.map(getValue("dollar_value")));
...
let sanitizeNames = fp.flowRight(_underscore, fp.toLower, fp.map(getValue("name")));
